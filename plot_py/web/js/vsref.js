...
//  }
  ...
  function myWin(day){
    newWin = open ("ftp://ftp.emc.ncep.noaa.gov/mmb/bzhou/VSREF/day", "displayWindow", "width=1000,height=800,menubar=yes,resizable=yes,scrollbars=yes,toolbar=yes,location=yes,status=yes");
  }
...
function load_image(frm){
  if(frm.elements["type"].selectedIndex < 9) begin=0;
  if(frm.elements["type"].selectedIndex == 4 ) begin=1;
  if(frm.elements["type"].selectedIndex == 10 ) begin=2;
  if(frm.elements["type"].selectedIndex == 11 ) begin=5;
  ...
  for(k=begin+1; k<13; k++){
    ...
    filename="../../"+frm.dy.options[frm.dy.selectedIndex].value+"/"+frm.prod.options[frm.prod.selectedIndex].value+".t"+frm.cyc.options[frm.cyc.selectedIndex].value+"z."+data+".gif";
    preload(filename);
  }
  show(0);
}
